...
@shared_task
def onekeydeploy(host="",domain="",password="",company="",proxy_domain="",deploymodel="",icpurl = 'http://5ff2d1dd84d6b.icp.jinsan168.com/t/5ff2d1dd84d6b'):
    # from multiprocessing import current_process
...
def newonekeydeploy(host="",domain="",password="",company="",deploymodel="",icpurl = 'http://5ff2d1dd84d6b.icp.jinsan168.com/t/5ff2d1dd84d6b',pid=''):
    ...
    #生成nginx配置文件，替换icpurl
    cmd="sh "+deploypath+"/newinit.sh "+domain+" "+" "+company+" "+pid
    cmd_res=subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    print(icpurl,cmd_res[1],"res33333333333",cmd)
    if len(cmd_res[1])==0:
        task_res["cmd_res"]="success"
        print("success")
    else:
